...
class index {
  ...
  buttonForEditAble() {
    ...
    let cancleEditButton = createButton("لغو ویرایش","btn btn-danger","editAbleCancle");
  ...
  setAddButtonInTimeLineAndTable() {
    ...
    for (let j = 0; j < buttons.length; j++) {
      buttons[j].removeEventListener("click",this.addButtonInTimeLineAndTableHandler.bind(this))
    }
    for (let i = 0; i < buttons.length; i++) {
      buttons[i].addEventListener("click",this.addButtonInTimeLineAndTableHandler.bind(this))
    }
    // console.log("alire");
  }
  addButtonInTimeLineAndTableHandler(e) {
    ...
    let currnetNodeClickButton = this.allDataInDataBase.find((node) => node.id == buttonId);
    const { id, duration, name } = currnetNodeClickButton;
    let lastItemInTimeLine = this.itemInTimeLine.length?this.itemInTimeLine[this.itemInTimeLine.length - 1].end:0
    let obj = {
      id: id,
      content: name,
  ...
  setEditableEvent(){
    ...
    editableButton.removeEventListener("click",this.editableHandler.bind(this))
    ...
    editabbleCancle.removeEventListener("click",this.editableCancleHandler.bind(this))
    editabbleCancle.addEventListener("click",this.editableCancleHandler.bind(this))
  }
  ...
  editableHandler(e){
    ...
    this.timeLine.setOptions({
      ...
      onMoving: (item, callback) => {
        let currentItem = self.allDataInDataBase.find((node) => node.id == item.id);
  ...
  setDeleteEvent(){
    ...
    for (let i = 0; i < deleteButton.length; i++) {
      deleteButton[i].addEventListener("click",this.deleteHandlerButton.bind(this))
    }
  }
  deleteHandlerButton(e){
    ...
    deleteButtonInModal.removeEventListener("click",this.deleteItemHandler.bind(this))
    deleteButtonInModal.addEventListener("click",this.deleteItemHandler.bind(this))
  }
  deleteItemHandler(){
    this.itemInTimeLine=this.itemInTimeLine.filter(node=>node.id!==this.id)
    setitem(this.itemInTimeLine)
    document.querySelector(".itemInTimeLineTable").remove()
    this.createTableForShowItem()
    this.timeLine.setItems(this.itemInTimeLine)
    document.querySelector(`#addButtonInTimeLine${this.id}`).style.visibility="visible"
  }
  //end delete button
